...
| `EPISTEMIC_GRAPH_RLS_DEFAULT_DENY` (EG-P0-6, feature `security`) | `0`/`false`/`no`/`off` (case-insensitive): opt OUT of the STRICT/default-deny posture back to the permissive posture (an unowned/undecodable/untagged-legacy row stays visible to all, the pre-WS-1b behavior). **Default `true` (STRICT/secure-by-default) as of WS-1b (2026-07-12)** — unset, or any value other than an explicit opt-out, resolves to STRICT (such a row is hidden unless explicitly `_visibility: "public"` or `_owner`-tagged); a fresh/greenfield deployment gets the secure posture with no config. **Migration note:** an existing deployment relying on the old permissive default must now either set `EPISTEMIC_GRAPH_RLS_DEFAULT_DENY=0` or backfill legacy unowned rows with an `_owner`/`_visibility: "public"` tag. See `eg_core::isolation::resolve_rls_default_deny` (the pure, unit-tested env resolver) and `IsolationLayer::rls_default_deny` |
...
| `EPISTEMIC_GRAPH_LAZY_OPEN_PAGE_SIZE` (CONCEPT:EG-KG.sharding.paged-lazy-open, L38 "paged adjacency", `redb` feature) | Switches `cold_offload::lazy_open` from a full inline rehydrate to `GraphRegistry::open_lazy_paged`: the graph becomes resident/queryable after just ONE bounded page of `<value>` nodes+edges, with the rest paged in by a background task (`page_in_remaining`) off the request path — closing the honest limitation that a lazily-opened graph previously always fully rehydrated on first touch regardless of size. A node not yet paged in still resolves correctly via the pre-existing per-node read-through; only a whole-graph scan sees an incomplete set until the background pass finishes. Backed by a genuinely SOURCE-bounded scan on the redb backend (`RedbBackend::read_graph_material_page_blocking` / `read_graph_dump_page`) — never collects the whole graph's rows into memory first, unlike the full-fetch-then-slice default any other backend gets. **Default `0` = paged open OFF** (the pre-existing `open_lazy` full-rehydrate path, byte-for-byte unchanged) |
