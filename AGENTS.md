...
`clamp(effective-cgroup-cpu/2, 1, 8)` (env `EPISTEMIC_GRAPH_REDB_SHARDS` overrides). **K=1 is the
...
- **Distributed planes (Phase 2, `raft`/`cluster` unless noted).** `PlacementCatalog`
  ...
  (an engine-owned typed `NativeMutationCommand::NodeInfo`, NOT
...
| `EPISTEMIC_GRAPH_ADVERTISED_CLIENT_ADDR` (CONCEPT:EG-KG.sharding.cluster-topology, ADR-1 / W1.1, `raft::config`) | This node's client-reachable address, self-reported into the durable cluster-topology store through an engine-owned typed Raft command and handed back by `Method::ClusterMembers`/`PlacementRoute.endpoints` — the engine-authoritative discovery that replaces the static hand-maintained `GRAPH_RAFT_GROUP_ENDPOINTS` client map. **Required whenever Raft peers are configured** (`EPISTEMIC_GRAPH_RAFT_NODE_ID`/`_PEERS` set) — config-contract style, like the transport secret: a clustered node refuses to start without it, since a discovering client would otherwise have no address to learn for this node beyond its own seed contact. Not read at all when Raft is not configured (single-node). |
...
| `EPISTEMIC_GRAPH_LINEAGE_KAFKA_BROKERS` (CA-15, `DEC-CA-03`/`DEC-CA-05`, feature `lineage-transport-kafka`, NOT in `full`) | Bootstrap-servers string for the OpenLineage Kafka transport (`src/server/lake/lineage_transport.rs`). When set, every `RunEvent` `EPISTEMIC_GRAPH_OPENLINEAGE_URL` would push over HTTP is ALSO published to the fixed `openlineage.events` topic, keyed by `run.runId` (normal key-hash partitioning — unlike `eg.cdc.<graph>`'s single-partition contract). Built on `eg_stream::sink::RawKafkaProducer` (feature `cdc-kafka`, reused — not a second `rdkafka` dependency, `file-ownership.yaml` `FO-CA-026`). **Unset ⇒ silent no-op**, same contract as the HTTP var; a producer-construction failure (bad broker string, unsupported SASL mode) is logged and this transport is left uninstalled, never a crash. |
| `EPISTEMIC_GRAPH_LINEAGE_KAFKA_SECURITY_PROTOCOL` / `_SASL_MECHANISM` / `_SASL_USERNAME` / `_SASL_PASSWORD` (CA-15, feature `lineage-transport-kafka`) | SASL/TLS producer config for the OpenLineage Kafka transport, mirroring `EPISTEMIC_GRAPH_CDC_KAFKA_*`'s shape (a separate env namespace — the two are independently configured producers). Normally unset: the broker is PLAINTEXT/no-auth until CA-51's security upgrade (`DEC-CA-03`). |
| `EPISTEMIC_GRAPH_LAKE_MATERIALIZE_INTERVAL_SECS` (INT-P2-3, feature `lake`, in `full`) | Positive interval, in seconds, for the periodic WAL/series→lakehouse materialization sweep: each tick lists every tsdb series and incrementally drains any new points into its Iceberg/Delta lake table. **`0`/unset ⇒ disabled** (the sweep never runs; a caller can still drive `LakeManager` directly, e.g. `drain_series`). A18 (fixed): the sweep no longer runs behind a client-carrier check at all — it is engine-internal system maintenance, not a client request, the same precedent `server::registry_reaper` and the cold-offload sweep already establish; see `reports/issue-register.md` (A18) for the before/after. |
| `EPISTEMIC_GRAPH_ICEBERG_ADDR` (INT-P2-3, feature `lake-rest`, in `full`, alias `--iceberg-addr`) | When set, a hand-rolled HTTP/1.1 listener (the SAME dependency-free idiom as `--metrics-addr`/`--obs-addr` — NO axum/hyper) binds this address (documented loopback `127.0.0.1:8181`) and serves the `iceberg.apache.org/rest-catalog-spec` surface — `GET /v1/config`, `GET /v1/namespaces[/…]`, `GET /v1/namespaces/{ns}/tables[/…]` (ListTables/LoadTable/HEAD-exists), `POST /v1/namespaces/{ns}/tables/{table}` (CommitTable, bridged to the engine's own compaction pass) — over the tables the `lake` materialization tier writes, so a standard Iceberg REST client (PyIceberg/Spark/Trino) can list + load them. **Unset, or a build without `lake-rest`, ⇒ no listener.** A18 (fixed): the shared carrier-check STUB that used to deny every `serve_with_security`-wired auxiliary surface unconditionally is gone — `s3-api` (SigV4), `sparql-http` (mutations, plus the SELECT/CONSTRUCT/ASK bearer/JWT leg), `kvcache-server` (bearer/JWT), and this surface itself now mint a real `CarrierAuthority` from their own protocol-native proof. This surface's carrier comes from a verified OAuth2 bearer against this deployment's configured tenant (`server::auth::mint_iceberg_carrier`, `resolve_iceberg_credential` reading `JwtValidator::from_env_iceberg()`); an unconfigured or invalid bearer still fails closed. `obs`, `federation-search`, and SPARQL's own read legs that have no `Method` RPC to dispatch through (`/nl`, Graph Store Protocol GET/HEAD) still have no protocol-native credential to verify and correctly stay fail-closed — that remains open pending a credential design for those surfaces. See `plans/_archive/au-eg-program/issue-register.md` (A18) for the per-surface disposition; the endpoint set and response shapes are exercised through the feature's own `serve()` (non-security) test suite in `src/server/lake/rest.rs`. |
| `GRAPH_SERVICE_SOCKET` | Path to the UDS socket |
| `GRAPH_SERVICE_SOCKET_MODE` (alias `--socket-mode`) | Octal file-mode applied to the UDS socket right after bind (e.g. `0600`, `0660`). **Default `0600`** (owner-only) — byte-for-byte the prior hardcoded behavior. Set this instead of an external `chmod` watcher when a non-root client container needs group access (e.g. a `fsGroup`-owned socket directory under `readOnlyRootFilesystem`). Validated at startup and **refused** if malformed or if it would grant `"other"` (world) access — this can only ever widen who may `connect()` via the group, never past it; a world-open mode is rejected outright rather than accepted. |
...
| `EPISTEMIC_GRAPH_REDB_GROUP_LINGER_US` | Positive adaptive group-commit micro-linger in microseconds (default `1000`). A shallow barrier batch — graph mutations and/or Raft log entries — waits once for concurrent writers to join the same fsync; durability remains commit-before-ack. |
| `EPISTEMIC_GRAPH_REDB_GROUP_SHALLOW` | Shallow durable-work threshold for the EG-024 micro-linger (default `32`, clamped 1..4096). The writer counts pending graph mutations plus Raft log entries; while that sum is below this value it lingers once, while a deeper batch commits immediately (adaptive — no added latency on a deep queue). |
| `EPISTEMIC_GRAPH_REDB_SHARDS` | **Sharded K-way durable writer (CONCEPT:EG-KG.backend.sharded-k-way-durable).** Number of independent `graph-<n>.redb` files/writer threads, overriding auto-size `clamp(effective-cgroup-cpu/2, 1, 8)` (clamped 1..64). Each graph routes to a fixed shard by `FNV-1a(sanitized_name) % K`; `K=1` uses canonical `graph-0.redb`. The value is fixed per durable store. **Ignored under an active Raft node** — there K == N and follows `EPISTEMIC_GRAPH_RAFT_GROUPS` instead (ADR-2 / W1.2, group *g* owns shard *g*), logged as a warning if set. Normal startup rejects the retired unindexed `graph.redb`; convert it offline with `migrate-shards --shards 1`. |
| `EPISTEMIC_GRAPH_REDB_FLUSH_THRESHOLD` | Per-shard auto-sized early-flush threshold. The writer flushes a `Pending` batch once it reaches the threshold, bounding RAM before the channel saturates. Default is half the durable-writer queue depth, clamped `256..16384`; a positive override may lower that bound but cannot raise it (after the hard `64..1_048_576` validation window). |
...
| `EPISTEMIC_GRAPH_MAX_INFLIGHT` | Server backpressure cap (default effective-cgroup `cpus*64`, clamped `64..8192`; positive overrides are clamped to that bound); excess → `BUSY` |
...
| `EPISTEMIC_GRAPH_VIZ_INTERACTIVE_ADDR` | **Interactive native-visualization HTTP listener (D-VZ-1 lane V3b, feature `viz-interactive`, alias `--viz-interactive-addr`).** When set (build `--features viz-interactive`), a hand-rolled HTTP/1.1 listener (the SAME dep-free idiom as `--metrics-addr`/`--obs-addr` — NO axum/hyper/websocket) binds this address (documented loopback `127.0.0.1:5090`) and serves `GET /` (a reference WebGPU client, WebGL2 fallback, honest "cannot accelerate this view" message if neither is available) and `GET /tile` (a binary viewport-tile protocol built on `eg_viz_kernels::lttb_reduce` over the SAME persistent `viz_engine::VizEngineState` — lane V4 — the RPC `Method::Viz` render path uses; a caller never gets two independently-drifting engines). **Unset, or a build without `viz-interactive`, ⇒ no listener** (a warning is logged if the addr is set but the feature is off). Separate from the RPC/metrics/obs transports |
...
| `EPISTEMIC_GRAPH_LEARNED_COST` (W4.12 phase 2, `eg-plan` feature `learned-cost`, CONCEPT:EG-KG.query.adaptive-reoptimization) | Runtime opt-in for the KAN-learned cardinality-estimate correction (`eg_plan::learned_cost`): `1` ⇒ the executor's per-op adaptive-reopt loop (`exec::run_with_adaptive_reopt`, auto-wired into every `Plan::execute`) additionally trains one small per-op-kind `KanEdgeFn` correction curve (eg-compute's polynomial-basis KAN — the SAME primitive W4.2's graphlearn link-predictor trains) online from each op's real `(estimated, actual)` cardinality pair, and `ModalityCardinality::rows_out` applies the learned correction to its static estimate — a bias learned on one query nudges later queries' plan-time estimates, not just that query's reactive re-plan. **Unset / anything but `1` ⇒ disabled** (checked fresh on every call, no caching) — a build compiled with `learned-cost` still ships byte-for-byte the pre-existing static-estimate behavior until this is also set. Requires the `learned-cost` cargo feature (`dep:eg-compute` + `eg-compute/graphlearn`); the eg-plan `query`/`cost-opt` kill-switch `EPISTEMIC_GRAPH_COST_OPT=0` also disables this (no per-op bookkeeping at all when cost-opt itself is off). The learned curves are process-wide (`LearnedCostStore::global`, persist across queries, not scoped to one `PlanCtx`/snapshot) and inspectable as serializable `CostCorrectionArtifact`s via `eg_plan::learned_cost_artifacts()` — see that module's doc for the "queryable artifact" surface and its A/B harness (`crates/eg-plan/tests/learned_cost_ab.rs`). |
| `EPISTEMIC_GRAPH_ENCRYPTION_KEY` | Encryption-at-rest key material (CONCEPT:EG-KG.sharding.row-level-security, feature `security`). When set, the redb durable **value** blobs (node/edge property + semantic store) are sealed with a pure-Rust ChaCha20-Poly1305 AEAD (RustCrypto — NO ring/openssl) — raw `.redb` bytes hold no plaintext properties. Keys stay plaintext so range scans work. **Default OFF / opt-in** (changes the on-disk format). `ValueCipher::from_env_checked` is the startup/KMS hook seam (swap it for a data-key fetch). A wrong key or malformed metadata fails closed before writer/listener admission (never silent plaintext) |
| `EPISTEMIC_GRAPH_ENCRYPTION_KEY_ID` | Stable, non-secret KMS object identifier pinned into each encrypted shard's authenticated canary and every backup manifest. Existing single-secret stores default to `legacy`; changing it is an explicit offline re-encryption boundary. |
| `EPISTEMIC_GRAPH_ENCRYPTION_KEY_VERSION` | Stable, non-secret key version pinned beside the encryption canary. Existing single-secret stores default to `1`; changing it is an explicit offline re-encryption boundary. |
| `EPISTEMIC_GRAPH_ENCRYPTION_REQUIRED` (GOC-16, `crypto::EncryptionRequiredMode`) | **Readiness posture for the encryption-key gap above.** Tri-state, same shape as `EPISTEMIC_GRAPH_REQUIRE_NODE_BINDING`: `off` = a missing key is accepted silently, byte-for-byte today's shipped behavior; `warn` (**unset/unrecognized ⇒ this, the shipped default**) = a missing key is accepted but logs once per `Shard::open` — fixes the PRIOR total silence (the `None` branch logged nothing at all); `on` = `Shard::open`/`RedbBackend::open` REFUSE with `Err` before any writer thread spawns or listener binds, propagated by the existing caller in `main.rs` into `eprintln!` + `exit(1)` — no new refusal idiom. A PRESENT `EPISTEMIC_GRAPH_ENCRYPTION_KEY` satisfies every posture identically; only an ABSENT key's handling varies |
