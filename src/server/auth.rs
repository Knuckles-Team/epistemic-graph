...
//! mutation nonces are carried into the durable MutationBatch replay ledger,
//! while read-only nonces are accepted by the transport replay ledger before
//! dispatch. Unknown envelope formats fail closed. This workspace intentionally has no downgraded protocol,
...
///   posture).
#[cfg(feature = "oidc")]
pub(crate) fn iceberg_bearer_scopes(claimed: &HashSet<String>) -> Result<Vec<String>, String> {
...
pub(crate) fn coarse_kg_admin_only(action: &str) -> bool {
...
#[derive(Debug, Clone)]
pub(crate) struct RequestContextPolicy {
    pub(crate) expected_audience: String,
    pub(crate) expected_tenant: String,
    pub(crate) expected_policy_version: String,
}
...
#[cfg(not(test))]
pub(crate) fn request_context_policy() -> Result<&'static RequestContextPolicy, String> {
...
/// mutation kernel stores in its replay ledger. The v2 envelope predates the
/// fixed-width contract and therefore permits a non-empty string; preserving
/// its full value through a domain-separated SHA-256 mapping keeps existing
/// signed clients valid while ensuring the kernel consumes exactly the MACed
/// wire value. Canonical 64-character hex nonces retain their byte identity.
fn kernel_nonce(wire_nonce: &str) -> Result<Nonce, String> {
    if wire_nonce.is_empty() {
        return Err("request nonce must not be empty".to_string());
    }
    if let Ok(parsed) = Nonce::parse(wire_nonce) {
        return Ok(parsed);
    }
    let mut input = Vec::with_capacity(17 + wire_nonce.len());
    input.extend_from_slice(b"eg/wire-nonce/v1\0");
    input.extend_from_slice(wire_nonce.as_bytes());
    Ok(Nonce::from_bytes(Sha256::digest(input).into()))
}
...
fn verify_envelope_v2_with(
    ...
    let wire_nonce = kernel_nonce(&envelope.nonce)?;
    // Mutations carry this authenticated nonce into MutationEnvelope and let
    // the kernel consume it atomically with the effect. A per-node transport
