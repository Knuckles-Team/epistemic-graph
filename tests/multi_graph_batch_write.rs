...
fn results_and_errors(resp: &Response) -> (serde_json::Map<String, serde_json::Value>, serde_json::Map<String, serde_json::Value>) {
    ...
    let results = v.get("results").and_then(|x| x.as_object()).cloned().unwrap_or_default();
    let errors = v.get("errors").and_then(|x| x.as_object()).cloned().unwrap_or_default();
    (results, errors)
}
...
async fn multi_graph_batch_write_fans_across_graphs() {
    ...
    for g in ["src:a", "src:b", "src:c"] {
        ...
        s.registry.create_graph(g, GraphType::Commons, None).unwrap();
    }

    let payload = batches_blob(vec![
        ("src:a", json!([{"op": "add_node", "id": "n-a", "properties": {"type": "Doc", "src": "a"}}])),
        ("src:b", json!([{"op": "add_node", "id": "n-b", "properties": {"type": "Doc", "src": "b"}}])),
        ("src:c", json!([{"op": "add_node", "id": "n-c", "properties": {"type": "Doc", "src": "c"}}])),
    ]);

    let resp = dispatch(
        &state,
        req(1, Method::MultiGraphBatchUpdate { batches_msgpack: payload }),
    )
    .await;
    ...
    for g in ["src:a", "src:b", "src:c"] {
        let r = results.get(g).unwrap_or_else(|| panic!("no result for {g}"));
        assert_eq!(r.get("added_nodes").and_then(|x| x.as_u64()), Some(1), "{g}");
    }
    ...
    assert!(!has_node(&state, 13, "src:a", "n-b").await, "no cross-graph leak");
    assert!(!has_node(&state, 14, "src:b", "n-a").await, "no cross-graph leak");
}
...
async fn multi_graph_batch_write_is_partial_success() {
    ...
    {
        let s = &mut *state.write().await;
        s.registry.create_graph("src:ok", GraphType::Commons, None).unwrap();
    }
    let payload = batches_blob(vec![
        ("src:ok", json!([{"op": "add_node", "id": "ok-1", "properties": {"type": "Doc"}}])),
        ("src:missing", json!([{"op": "add_node", "id": "x-1", "properties": {"type": "Doc"}}])),
    ]);
    let resp = dispatch(
        &state,
        req(2, Method::MultiGraphBatchUpdate { batches_msgpack: payload }),
    )
    .await;
    let (results, errors) = results_and_errors(&resp);
    assert!(results.contains_key("src:ok"), "healthy sub-batch applied");
    assert!(errors.contains_key("src:missing"), "missing graph surfaced as error");
...
async fn single_graph_multi_batch_equals_plain_batch() {
    ...
    {
        let s = &mut *state.write().await;
        s.registry.create_graph("src:solo", GraphType::Commons, None).unwrap();
    }
    let payload = batches_blob(vec![(
        "src:solo",
        json!([
    ...
    let resp = dispatch(
        &state,
        req(3, Method::MultiGraphBatchUpdate { batches_msgpack: payload }),
    )
    .await;
    let (results, errors) = results_and_errors(&resp);
    assert!(errors.is_empty(), "unexpected errors: {errors:?}");
