...
flowchart TB
    ...
    subgraph Subsystems["Cross-cutting subsystems"]
        BROKER["Message broker (eg-core/broker)"]
        OBS["Observability (eg-tsdb + obs)"]
        MEM["Agent-memory (eg-core)"]
        KVC["KV-cache tiering (eg-kvcache)"]
    end
...
| redis | RESP2/RESP3 core command set | EG-174 | KV surface |
| s3 | S3 REST + SigV4-lite | EG-176 | BLOB CAS |
| amqp | AMQP 0.9.1 | EG-275 | message broker |
| mqtt | MQTT 3.1.1 / 5.0 | EG-281 | message broker |
...
**replayable append-log streams** with offset-addressed replay + retention (EG-283). The AMQP, MQTT, and
STOMP wire adapters (EG-275/281/282) are three front doors onto this one broker.
...
`/api/v1/query[_range]` (EG-172), an **O2/ES-shaped `_search`** over the Parquet segments unioned with the
hot tsdb series + BM25 index (EG-162), and **distributed traces** — OTLP span ingest on `/v1/traces` with
trace-tree assembly, tag/service/duration search, and service-dependency-graph derivation (EG-163). A
**super-cluster federated search** (EG-243) fans a read out to a registry of peer engines and merges +
re-ranks the partials, tolerating a slow/dead peer. Gated `obs`, out of `pi`.
...
index (EG-263), **format I/O** (GeoJSON/WKB/GPX; EG-264), **map tiling** (XYZ/TMS + Mapbox Vector Tiles;
EG-265), **routing/isochrones/TSP** (EG-266), and **map-anchored task tracking** (`:GeoTask`; EG-267). It
surfaces through SQL `st_*` and a **GeoSPARQL** vocabulary (EG-261). Folded into `node`/`full`, out of `pi`.
...
Depends on eg-tsdb; folded into `node`/`full`, out of `pi`.
...
same reactive substrate that drives continuous queries and watches. Gated `stream`, folded into `node`/`full`.
...
compressed-RAM tier, and a cold redb/blob tier, with automatic promotion/demotion on access + capacity
...
graph primitives. All in eg-core, Pi-safe.
