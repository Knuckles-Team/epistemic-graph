...
| `m3.online-reshard-delta` | `UNIT-PROVEN` | `src/server/persistence/redb_backend.rs#super::online_reshard::delta_flip_purge` | `import(delta) committed -> catalog flip durable ->` | Current source and the focused no-loss reshard fixture cover the snapshot-plus-delta quiesce and crash-ordering path. |
