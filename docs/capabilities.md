...
| Calibrated causal reasoning — linear-Gaussian SCM with genuine Pearl do-calculus (`observe`/`intervene`/`counterfactual`, each returning a calibrated credible interval) + provenance-aware retrieval ranking | ✅ | facade `epistemic-causal` (opt-in, not in `full`) | CONCEPT:EPI-P3-3; `eg-epistemic::{causal,ranking}`. Facade-reachable via `Method::CausalEstimate` (the do-`intervene` op) and `Method::RankByProvenance`; the crate's `observe`/`counterfactual` variants remain crate-internal (documented follow-up) |
