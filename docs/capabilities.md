...
| W4.5 named graph-projection catalog: `gds.graph.project`/`.drop`/`.exists`/`.list` (Neo4j-GDS-equivalent) — every OTHER `gds.*` procedure accepts an optional `graphName` config key that REUSES the materialized projection instead of re-scanning the view, invalidated on a relevant write via the SAME W1.6 `DepClock` the result cache's dependency-scoped entries use (not a parallel scheme) | ✅ | `eg-core::projection_catalog::ProjectionCatalog` (feature `result-cache`); `crates/eg-query/src/cypher/gds.rs`'s `project_named` (CONCEPT:EG-KG.query.named-graph-projection-catalog) |
| W4.5 approximate sketches: `APPROX_DISTINCT(expr)` (HyperLogLog), `APPROX_FREQUENCY(expr, probe)` (Count-Min Sketch, never underestimates), `MINHASH_SIGNATURE(expr)` + `MINHASH_SIMILARITY(sig1, sig2)` (MinHash Jaccard estimate) as SQL aggregate/scalar UDFs; the SAME `HyperLogLog` feeds `eg-plan`'s `PlanStats::distinct_stats`, sharpening an equality predicate's selectivity from a fixed heuristic to `1/distinct_count` | ✅ | pure-Rust `eg_compute::sketch` (no heavy dep), always-on under `sql`; `crates/eg-query/src/sql/sketch_udfs.rs`; `crates/eg-plan/src/cost.rs`'s `DistinctStats` (CONCEPT:EG-KG.query.approx-distinct-cardinality) |
