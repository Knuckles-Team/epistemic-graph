...
fn result_note(d: &MethodDescriptor, declared: Option<&Declared>) -> String {
    let id = d.id.as_str();
    let schema = format!("{}#/methods/{id}", schema::result_document_path(d.domain));
    match declared.map(Declared::single) {
        None => "unclassified - the contract declares no result for this method".to_string(),
        Some(None) => format!("one declared body per request op ({schema})"),
        Some(Some(body)) => body_note(body, &schema),
    }
}

/// The note for one declared body: its encoding, and its reason when caller-shaped.
fn body_note(body: &Body, schema: &str) -> String {
    match body.dynamic {
        Some(reason) => format!(
            "ResultPayload::{}, caller-shaped: {} ({schema})",
            body.encoding,
            reason.summary()
        ),
        None => format!("ResultPayload::{} ({schema})", body.encoding),
    }
}
...
fn push_send(out: &mut String, d: &MethodDescriptor, declared: Option<&Declared>) {
    ...
    out.push('\n');
    let _ = writeln!(out, "    Result: {}.", result_note(d, declared));
