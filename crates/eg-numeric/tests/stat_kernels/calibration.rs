...
use eg_numeric::calibration::metrics::{
    brier_score, classwise_ece, log_loss, reliability, top_label_reliability,
};
use eg_numeric::calibration::scaling::{calibrate_rows, negative_log_likelihood};
use eg_numeric::calibration::{
    fit_isotonic, fit_isotonic_weighted, fit_scaling, BinCount, LabelledScores, ProbabilityFloor,
    ProbabilityMatrix, ScalingFamily, ScalingOptions, ScoreMatrix,
};
...
fn temperature_scaling_recovers_a_planted_temperature() {
    ...
    let fitted = fit_scaling(
        &labelled(&rows, &labels),
        ScalingFamily::Temperature,
        ScalingOptions::default(),
    )
    .unwrap();
    assert_close(
        fitted.temperature().unwrap(),
        3.0,
        0.2,
        "planted temperature 3",
    );
    ...
    .unwrap();
    assert_close(
        identity.temperature().unwrap(),
        1.0,
        0.08,
        "calibrated data keeps T = 1",
    );
}
...
fn vector_and_matrix_scaling_recover_planted_probabilities() {
    ...
    for family in [ScalingFamily::Vector, ScalingFamily::Matrix] {
        ...
        assert!(
            error < 0.03,
            "{family:?}: mean max probability error {error}"
        );
    }
    assert_close(
        negative_log_likelihood(&data),
        fit_scaling(&data, ScalingFamily::Vector, options)
            .unwrap()
            .nll_before(),
        0.0,
        "identity NLL",
...
fn isotonic_regression_pools_violators_and_ties() {
    ...
    assert_eq!(
        fit.blocks(),
        vec![(1.0, 2.0, 0.5), (3.0, 4.0, 0.5), (5.0, 5.0, 1.0)]
    );
...
fn isotonic_recovers_a_planted_curve_and_removes_calibration_error() {
    ...
    let mapped: Vec<f64> = fresh_scores
        .iter()
        .map(|&s| fit.predict(s).unwrap())
        .collect();
    let after = reliability(&mapped, &fresh_outcomes, bins).unwrap();
    assert!(
        after.expected_calibration_error < 0.02,
        "isotonic ECE {}",
...
fn reliability_table_matches_a_hand_computed_example() {
    let report = reliability(
        &[0.1, 0.1, 0.9, 0.9, 1.0],
        &[false, true, true, true, true],
        BinCount::new(10).unwrap(),
    )
    .unwrap();
...
fn brier_log_loss_top_label_and_classwise_metrics() {
    ...
    assert_close(
        brier_score(&probabilities, &[0, 0]).unwrap(),
        (0.14 + 1.04) / 2.0,
        1e-15,
        "brier",
    );
    ...
    assert_close(
        log_loss(&probabilities, &[0, 0], floor).unwrap(),
        expected_loss,
        1e-15,
        "log loss",
    );
    ...
    assert_close(
        top.expected_calibration_error,
        (0.3 + 0.6) / 2.0,
        1e-15,
        "top-label ECE",
    );
