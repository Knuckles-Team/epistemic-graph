...
use crate::common::{assert_all_close, assert_at_least_within, assert_close, categorical, level, rng, uniform, uniform_in};
use eg_numeric::calibration::ProbabilityMatrix;
use eg_numeric::conformal::{
    acceptability_score, aps_label_scores, aps_scores, binary_conformal, lac_scores, mondrian_conformal,
    realised_coverage_interval, split_conformal, weighted_split_conformal, AdaptiveConformal, BinarySet,
    ClassThreshold, PredictionSet, RapsPenalty, Threshold,
};
...
fn split_conformal_uses_the_exact_order_statistic() {
    ...
    assert_eq!((quantile.n_calibration(), quantile.alpha()), (9, level(1, 10)));
    ...
    assert_eq!(split_conformal(&scores, level(1, 2)).unwrap().threshold(), Threshold::Finite(5.0));
    assert!(split_conformal(&[], level(1, 10)).is_err() && split_conformal(&[f64::NAN], level(1, 10)).is_err());
    let (lo, hi) = realised_coverage_interval(level(1, 10), 99, level(1, 20)).unwrap().unwrap();
    ...
    assert_eq!(realised_coverage_interval(level(1, 10), 5, level(1, 20)).unwrap(), None);
...
fn split_conformal_regression_coverage_is_nominal_over_seeded_repeats() {
    ...
    for seed in 0..repeats {
        ...
        let quantile = split_conformal(&heteroscedastic_scores(&mut generator, 199), alpha).unwrap();
        let test = heteroscedastic_scores(&mut generator, 1000);
        let covered = test.iter().filter(|&&s| quantile.threshold().admits(s)).count();
    ...
    assert!(mean <= 0.9 + 1.0 / 200.0 + 4.0 * sd / 10.0, "coverage {mean} is not tight");
}
...
fn aps_raps_and_lac_scores_match_closed_forms() {
    ...
    assert_all_close(&tied, &[0.4, 0.8, 1.0], 1e-15, "ties rank the lower class first");
    ...
    assert!(acceptability_score(&[0.7], &[]).is_err() && acceptability_score(&[0.7], &[1]).is_err());
    assert!(aps_scores(&[0.5, 0.5], 1.5, RapsPenalty::none()).is_err() && RapsPenalty::new(-1.0, 0).is_err());
}
...
fn classified(generator: &mut ChaCha8Rng, n: usize, classes: usize) -> Classified {
    let mut data = Classified { probabilities: Vec::new(), labels: Vec::new(), u: Vec::new() };
    for _ in 0..n {
        let z: Vec<f64> = (0..classes).map(|_| uniform_in(generator, -3.0, 3.0)).collect();
...
fn aps_coverage(seed: u64, penalty: RapsPenalty, randomised: bool) -> f64 {
    ...
    let u = if randomised { calibration.u.clone() } else { vec![1.0; 500] };
...
fn aps_and_raps_sets_cover_planted_labels() {
    ...
    let cases = [(RapsPenalty::none(), true), (RapsPenalty::none(), false), (RapsPenalty::new(0.05, 2).unwrap(), true)];
    for (penalty, randomised) in cases {
        let mean = (0..repeats).map(|seed| aps_coverage(3000 + seed, penalty, randomised)).sum::<f64>() / repeats as f64;
...
fn acceptability_coverage(seed: u64) -> f64 {
    ...
    let calibration: Vec<f64> = draw(300).iter().map(|(s, a)| acceptability_score(s, a).unwrap()).collect();
    let threshold = split_conformal(&calibration, level(1, 5)).unwrap().threshold();
    let test = draw(500);
    let met = test.iter().filter(|(s, a)| PredictionSet::from_scores(s, threshold).meets(a)).count();
    met as f64 / 500.0
}
...
fn acceptability_sets_meet_the_acceptable_options_at_nominal_rate() {
    let repeats = 50;
    let mean = (0..repeats).map(|seed| acceptability_coverage(4000 + seed)).sum::<f64>() / repeats as f64;
        ...
        let mut draw = |n: usize| -> (Vec<Vec<f64>>, Vec<usize>) {
            (0..n)
                .map(|_| {
                    ...
                    let row: Vec<f64> = (0..3).map(|k| uniform(&mut generator) + if k == label { 0.0 } else { 0.3 }).collect();
                    (row, label)
                })
                .unzip()
        };
...
fn mondrian_below_minimum_admits_the_class_without_a_claim() {
    ...
    let labels = [0, 0, 0, 1];
    let mondrian = mondrian_conformal(&scores, &labels, 2, level(1, 2), SampleGate::new(2).unwrap()).unwrap();
    assert!(!mondrian.fully_calibrated());
    assert_eq!(mondrian.thresholds()[1], ClassThreshold::BelowMinimum { n: 1, n_min: 2 });
    assert_eq!(mondrian.prediction_set(&[0.15, 100.0]).unwrap().options(), &[0, 1]);
    assert_eq!(mondrian.prediction_set(&[0.35, 100.0]).unwrap().options(), &[1]);
    assert!(mondrian.prediction_set(&[0.1]).is_err());
    assert!(mondrian_conformal(&scores, &[0, 0, 0, 2], 2, level(1, 2), SampleGate::new(1).unwrap()).is_err());
}
...
fn binary_conformal_sets_follow_per_label_thresholds() {
    ...
    let binary = binary_conformal(&scores, &outcomes, level(1, 5), SampleGate::new(4).unwrap()).unwrap();
    let cases = [(0.95, BinarySet::Positive), (0.5, BinarySet::Empty), (0.6, BinarySet::Positive), (0.4, BinarySet::Negative)];
    for (p, expected) in cases {
        assert_eq!(binary.predict(p).unwrap(), expected, "p = {p}");
    }
    let wide = binary_conformal(&scores, &outcomes, level(1, 10), SampleGate::new(4).unwrap()).unwrap();
    assert_eq!(wide.predict(0.5).unwrap(), BinarySet::Both);
    let gated = binary_conformal(&scores, &outcomes, level(1, 5), SampleGate::new(5).unwrap()).unwrap();
...
fn weighted_conformal_reduces_to_split_and_follows_the_weights() {
    ...
    assert_eq!(equal.threshold(), split_conformal(&scores, alpha).unwrap().threshold());
    let heavy_tail: Vec<f64> = scores.iter().map(|&s| if s > 0.5 { 10.0 } else { 0.1 }).collect();
    let shifted = weighted_split_conformal(&scores, &heavy_tail, 1.0, alpha).unwrap();
    let (Threshold::Finite(plain), Threshold::Finite(weighted)) = (equal.threshold(), shifted.threshold()) else {
        panic!("finite thresholds expected");
    };
    assert!(weighted > plain, "weights on large scores raise the threshold");
    assert_eq!(weighted_split_conformal(&[0.3], &[1.0], 100.0, alpha).unwrap().threshold(), Threshold::Infinite);
...
fn adaptive_conformal_meets_its_long_run_bound_under_drift() {
    ...
    let mut window: Vec<f64> = (0..200).map(|_| drifting_score(&mut generator, 0)).collect();
    ...
    for t in 0..3000 {
        ...
        assert!(gap <= report.miscoverage_bound + 1e-12, "t = {t}: gap {gap} above bound");
        window.remove(0);
        window.push(score);
    }
    let report = adaptive.report();
    ...
    assert!(static_misses as f64 / 3000.0 > 0.5, "the static threshold should fail under drift");
    assert!(AdaptiveConformal::new(alpha, 0.0).is_err());
    assert_eq!(AdaptiveConformal::new(alpha, 1.0).unwrap().report().miscoverage_bound, f64::INFINITY);
}
