...
pub struct Generator {
    choices: ChoiceSampler,
}

fn generator_from_rng(rng: ChaCha20Rng) -> Generator {
    Generator {
        choices: ChoiceSampler { rng },
    }
}
...
impl Generator {
    ...
    pub fn new(seed: u64) -> Self {
        generator_from_rng(ChaCha20Rng::seed_from_u64(seed))
    }
    ...
    pub fn from_entropy() -> Self {
        generator_from_rng(ChaCha20Rng::from_entropy())
    }
}

impl Deref for Generator {
    type Target = ChoiceSampler;
...
impl Generator {
    ...
    pub fn normal(&mut self, loc: f64, scale: f64, size: usize) -> Vec<f64> {
        ...
        (0..size)
            .map(|_| dist.sample(&mut self.choices.rng))
            .collect()
    }
    ...
    pub fn integers(&mut self, low: i64, high: i64, size: usize) -> Vec<i64> {
        (0..size)
            .map(|_| self.choices.rng.gen_range(low..high))
            .collect()
    }
...
/// Choice and permutation methods for [`Generator`]. The RNG remains in this
/// single state holder, so moving these methods does not alter stream order.
