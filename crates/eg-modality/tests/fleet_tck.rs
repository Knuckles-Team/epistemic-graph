...
fn fleet_parity_table_and_first_class_pilots() {
    ...
    assert_eq!(tensor.pass_count(), 9, "tensor: {}", tensor.summary());
    assert_eq!(tensor.na_count(), 3, "tensor: {}", tensor.summary());

    let geo = tck_report::<Geometry>();
    assert_eq!(geo.covered_count(), 12, "geo: {}", geo.summary());
    assert_eq!(geo.pass_count(), 9, "geo: {}", geo.summary());
    assert_eq!(geo.na_count(), 3, "geo: {}", geo.summary());
}
