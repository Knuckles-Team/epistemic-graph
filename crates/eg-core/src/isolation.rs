...
// ── Graph policy lease (GRAPH-POLICY-LEASE-CONTRACT.md) ─────────────────────
//
...
/// `grants: Vec<Grant>` is already order-stable — see [`policy_digest_bytes`]).
#[cfg(feature = "security")]
...
/// Canonical (deterministic across processes) encoding of a policy's
/// authorization-relevant content, used ONLY to compute [`PolicySnapshot::digest`]
/// — never `RbacStore::save`'s own `state_digest`, which is raw `serde_json`
/// over `RbacPolicy`'s `HashMap<String, Role>` roles field and therefore
/// non-deterministic across processes (contract §2.4, Finding 7/R5). `roles()`
/// is re-keyed into a `BTreeMap` here for exactly that reason; `grants()`
/// returns an already order-stable `&[Grant]` (a `Vec`, not a `HashMap`), so
/// no further canonicalization is needed there.
///
/// Implementation note: the contract specifies a CBOR encoding; `eg-core` has
/// no CBOR dependency today (only `serde_json`, already linked
/// unconditionally), and pulling one in purely for this digest was judged out
/// of proportion to the feature. Canonical `serde_json` over this
/// `BTreeMap`-keyed, digest-field-free structure satisfies the contract's
/// actual requirement — "deterministic across process restarts and across
/// the two independent computation sites" (§9) — the concrete byte layout is
/// explicitly left open there.
#[cfg(feature = "security")]
fn policy_digest_bytes(policy: &crate::rbac::RbacPolicy) -> Vec<u8> {
    #[derive(serde::Serialize)]
    struct CanonicalPolicy<'a> {
        roles: std::collections::BTreeMap<&'a str, &'a crate::acl::Role>,
...
#[cfg(feature = "security")]
fn policy_digest(policy: &crate::rbac::RbacPolicy) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"eg/rbac-policy-snapshot/v1\0");
    hasher.update(policy_digest_bytes(policy));
    hex::encode(hasher.finalize())
}

/// `SHA-256("eg/rbac-identity-snapshot/v1\0" || canonical(identities))`
/// (contract §2.5) — a SECOND, independent digest from `policy_digest`
/// because row visibility (`can_see_row`) additionally consults the
/// identity/manager-hierarchy state, which a policy-only digest would miss
/// (a role reassignment or manager-hierarchy edit that changes row
/// visibility without touching a single grant). The identity map is already
/// a `BTreeMap<String, AgentIdentity>` (rbac_persist.rs), so it is already
/// deterministic to serialize — no re-keying needed, unlike the policy's
/// `roles` field above.
#[cfg(feature = "security")]
fn identity_digest(identities: &std::collections::BTreeMap<String, AgentIdentity>) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"eg/rbac-identity-snapshot/v1\0");
    hasher.update(serde_json::to_vec(identities).unwrap_or_default());
    hex::encode(hasher.finalize())
}
...
/// [`GraphPolicyLease`] captures a point-in-time identities snapshot (via
/// [`crate::rbac_persist::RbacPolicyStore::load`]) rather than borrowing an
...
fn can_see_row_for_identities(
    ...
    if is_manager_of_for_identities(identities, agent_id, owner) {
        return true;
    }
    false
}
...
pub enum MintLeaseError {
    ...
    /// digest from (item 13).
    StoreUnavailable,
    /// The durable store is bound but could not be read (I/O/serde failure,
    /// or a store whose `current_version()` fails closed via the trait
    /// default).
    StoreUnreadable,
}
...
impl std::fmt::Display for MintLeaseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MintLeaseError::AccessDenied => write!(f, "graph policy lease denied"),
...
// registered actor and hand it straight to `mint_graph_policy_lease`, which
...
// mitigation actually available here: a wrapper `mint_graph_policy_lease`
...
// `mint_graph_policy_lease`, and no wrapper type changes that. What this DOES
...
/// used ONLY to compute the [`MintAuthorization`] HMAC. Mirrors
/// [`policy_digest_bytes`]'s approach immediately above (a purpose-built,
...
fn mint_authorization_mac(
    ...
    mac.update(b"eg/mint-graph-policy-lease-authorization/v1\0");
    mac.update(&mint_authorization_claims_bytes(claims));
    Ok(mac)
}

/// Capability token gating [`IsolationLayer::mint_graph_policy_lease`]
...
/// Staleness error from [`GraphPolicyLease::validate_before`]/`validate_after`/
...
impl std::fmt::Display for LeaseStaleError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LeaseStaleError::Stale => write!(f, "graph policy lease is stale"),
        }
    }
}
...
/// obtain one is [`IsolationLayer::mint_graph_policy_lease`] — see that
/// method's doc for why non-constructibility is a hard security requirement
/// (T3), not a style choice.
#[cfg(feature = "security")]
pub struct GraphPolicyLease {
    access: AccessLevel,
    actor: String,
    tenant: String,
    graph: String,
    policy_snapshot: PolicySnapshot,
    coarse_acl: GraphPolicyCoarseAcl,
    row_policy_identity_digest: String,
}

#[cfg(feature = "security")]
impl GraphPolicyLease {
    ...
    pub fn coarse_acl(&self) -> GraphPolicyCoarseAcl {
        self.coarse_acl
    }
    ...
    fn reload_and_verify_fresh(
        ...
        (
            ...
            std::collections::BTreeMap<String, AgentIdentity>,
        ),
        LeaseStaleError,
    > {
        let (policy, identities, _bootstrap) = store
            .load()
            .map_err(|_| LeaseStaleError::StoreUnavailable)?;
        if policy_digest(&policy) != self.policy_snapshot.digest
            || identity_digest(&identities) != self.row_policy_identity_digest
        {
    ...
    /// this lease's actor may see, using a fresh reload of the durable store
    ...
    pub fn filter_view(
        ...
        let (_policy, identities) = self.reload_and_verify_fresh(store)?;
        if matches!(self.coarse_acl, GraphPolicyCoarseAcl::System) {
        ...
        let hidden: HashSet<String> = view
            ...
            .keys()
            .filter_map(|id| {
                if can_see_node_for_identities(&identities, self.actor.as_str(), view, id) {
                    None
                } else {
                    Some(id.clone())
                }
...
impl IsolationLayer {
    ...
    ///
    /// This remains the general-purpose, directly-callable Rust-level primitive
    /// used by trusted in-process call sites across this crate to seed a fresh
    /// `IsolationLayer` with an `AgentRole::System` test/bootstrap identity.
    /// Served `Method::RegisterIdentity` requests must use
    /// [`Self::try_register_agent_from_request`] instead: the served entrypoint
    /// keeps the one-time System authority in
    /// [`Self::try_bootstrap_system_identity`] and therefore cannot turn the
    /// signer registry's structural self-registration allowance into a second
    /// System grant after genesis.
    ...
    /// Register an identity from the served, signer-authenticated
    /// `Method::RegisterIdentity` path.
    ///
    /// The dedicated bootstrap arm is the only served authority that may write
    /// an `AgentRole::System` identity.  `try_bootstrap_system_identity` performs
    /// the durable pending-state check and exact genesis shape validation; this
    /// arm rejects both a still-pristine layer and System identities before
    /// reaching the general-purpose registration primitive.  Keeping that
    /// primitive unrestricted preserves trusted in-process fixtures and
    /// internal seed paths, while preventing a signer with `may_grant_system`
    /// from using a non-bootstrap envelope to self-register as System after
    /// genesis.
    ...
    /// The ONLY function that may construct a [`GraphPolicyLease`]
    ...
    /// the SAME `store.load()` read (§2.4/§2.5) so a not-yet-persisted
    ...
    /// requires AND hardened beyond it — non-constructible `GraphPolicyLease`,
    ...
    #[cfg(feature = "security")]
    pub fn mint_graph_policy_lease(
        &self,
        auth: &MintAuthorization,
        graph_name: &str,
        access: AccessLevel,
    ) -> Result<GraphPolicyLease, MintLeaseError> {
        ...
        let actor = claims.agent_id.as_str();
        let tenant = claims.tenant.as_str();
        // Step 2: unregistered actor ⇒ no lease (matches `check_access`'s
        // `None ⇒ false`).
        let identity = self.agents.get(actor).ok_or(MintLeaseError::UnknownActor)?;
        // Step 3: the EXACT `check_access` decision — System bypass, else
        // `RbacPolicy::evaluate`. Never a second, laxer path.
        let coarse_acl = if identity.role == AgentRole::System {
            GraphPolicyCoarseAcl::System
        } else {
            let ctx = crate::acl::ResourceContext::graph(graph_name);
            let allowed = matches!(
                self.rbac
            ...
            GraphPolicyCoarseAcl::RbacAllow
        };
        // Step 4: the digest MUST come from the durable store, never from
        // `self.rbac`/`self.agents` in-memory — a not-yet-persisted mutation
        // must never mint a lease a concurrent reload can't reproduce (§3
        // step 4, §8 item 13).
        let store = self
            .persist
            .clone()
            .ok_or(MintLeaseError::StoreUnavailable)?;
        let (policy, identities, _bootstrap) =
            store.load().map_err(|_| MintLeaseError::StoreUnreadable)?;
        // R5/§9: `version` is audit/display-facing only, never the
        // staleness ground truth (`digest` is) — but a store that cannot
        // report it fails the mint closed rather than substitute a
        // plausible-looking `0` (§2.4's requirement on the trait method's
        // OWN default body applies with equal force here, at the one call
        // site that consumes it).
        let version = store
            .current_version()
            .map_err(|_| MintLeaseError::StoreUnreadable)?;
        let digest = policy_digest(&policy);
        let row_policy_identity_digest = identity_digest(&identities);
        Ok(GraphPolicyLease {
            access,
            actor: actor.to_string(),
            tenant: tenant.to_string(),
            graph: graph_name.to_string(),
            policy_snapshot: PolicySnapshot { version, digest },
            coarse_acl,
            row_policy_identity_digest,
        })
    }
