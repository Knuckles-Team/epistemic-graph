...
use crate::parser::{Fragment, RawDocument, RawSelection};
use crate::resolver::Variables;
use crate::schema::{validate_field_name, validate_type_name, Schema};
...
impl FederatedSchema {
    ...
    pub fn to_federation_sdl(&self) -> String {
        ...
        self.render_query_root(&mut out);
        for (name, t) in &self.base.types {
            self.render_entity_type(&mut out, name, t);
        }
        out
    }

    /// The `type Query { … }` root: one `[Type]` field per derived type. Split
    /// out of [`Self::to_federation_sdl`] (extract-method) so each function
    /// stays within the per-function complexity cap.
    fn render_query_root(&self, out: &mut String) {
    ...
    /// One federated entity type block: its `@key`/`@shareable` directives,
    /// `id: ID!`, scalar fields, and edge fields. Split out of
    /// [`Self::to_federation_sdl`] (extract-method).
    fn render_entity_type(&self, out: &mut String, name: &str, t: &crate::schema::ObjectType) {
        out.push_str(&format!("type {name}"));
        self.render_entity_directives(out, name);
        out.push_str(" {\n");
    ...
    /// A type's `@key(fields: "…")`/`@shareable` directive suffix. Split out of
    /// [`Self::render_entity_type`] (extract-method).
    fn render_entity_directives(&self, out: &mut String, name: &str) {
        let Some(meta) = self.entities.get(name) else {
            return;
        };
        for k in &meta.keys {
            if k.resolvable {
...
pub(crate) fn resolve(
    ...
    for sel in &doc.selections {
        ...
        match rf.name.as_str() {
            "_service" => {
                data.insert(rf.alias.clone(), entities::resolve_service(&fed, rf));
            }
            "_entities" => {
                data.insert(
                    rf.alias.clone(),
                    entities::resolve_entities(view, &fed, rf, &frags, vars)?,
                );
            }
            "__typename" => {
...
mod entities;
mod sdl_scanner;
use sdl_scanner::scan_directives;
...
mod tests {
    ...
    fn federation_entities_by_property_key_eg295() {
        ...
        let repr: Map<String, Value> = json!({ "__typename": "Person", "email": "b@x.io" })
            ...
            .unwrap()
            .clone();
        let (id, val) = entities::lookup_entity(&v, &fed, "Person", &repr)
            .unwrap()
            .unwrap();
        assert_eq!(id, "bob");
    ...
    fn federation_does_not_resolve_reference_only_keys() {
        ...
        let repr = json!({ "__typename": "Person", "email": "b@x.io" })
            ...
            .unwrap()
            .clone();
        assert!(entities::lookup_entity(&v, &fed, "Person", &repr)
            .unwrap()
            .is_none());
    }
