//! GPU-accelerable batch-distance dispatch seam (CONCEPT:EG-3.5 seam / EG-3.6 CUDA).
//!
...
/// (CONCEPT:EG-3.5). `rows_flat` is `n * dim` f32 (row `i` = `rows_flat[i*dim..(i+1)*dim]`);
...
/// Integer metric code shared by the CPU scalar path and the CUDA kernel (CONCEPT:EG-3.6)
...
/// Score `query` against `rows_flat` on the ACTIVE backend (CONCEPT:EG-3.5) — the GPU
...
/// The active backend (CONCEPT:EG-3.5). Selects the CUDA backend when it is compiled AND a
...
// ── CONCEPT:EG-3.6 — the real CUDA backend ───────────────────────────────────

#[cfg(feature = "gpu-cuda")]
pub mod cuda {
    //! REAL CUDA distance backend (CONCEPT:EG-3.6). Compiles the batch-distance CUDA-C
    ...
    /// The batch-distance kernel, CUDA-C (CONCEPT:EG-3.6). One thread scores one row; the
...
"#;
    ...
    impl CudaBackend {
        /// Initialise device `ordinal`, NVRTC-compile the kernel, and load it (CONCEPT:EG-3.6).
