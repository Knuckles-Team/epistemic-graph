...
impl StatechartStore {
    ...
    pub fn send_event(
        ...
        // A firing transition. Build the RESULTING instance image up front so the
        // MutationBatch is content-addressed by the exact bytes we will persist (the
        // key that makes a replayed Raft proposal an idempotent no-op), then commit
        // the row change and its terminal batch/version/fence/idempotency/outbox
        // evidence through the SAME `eg-mutation-store` gateway `eg-jobs` uses, on one
        // redb `WriteTransaction`. Inside that txn the row is re-read and its OCC
        ...
        // gateway. A well-defined no-op returned above; it never reaches here.
        let now = now_ms();
        let mut next = instance.clone();
        next.state = outcome.next_state.clone();
        ...
        next.status = if def.is_final(&next.state) {
            InstanceStatus::Final
        } else {
            InstanceStatus::Active
        };
