//! `blob` domain — pyo3 stub (Wave 0, `EG-PYENGINE-PLAN.md` §4.1). This
//! file exists ONLY so `crates/eg-pyengine/src/lib.rs`'s module tree compiles
//! and `PyEngine::blob()` has a stable, already-registered return type
//! before the `blob` Wave-1 lane starts — that lane's own first commit is
...
//! macro DSL, no generic `DomainOps<T>` to produce this file; the `blob`
...
/// One in-process handle to the `blob` surface, sharing the SAME
/// registry + authority the `Engine` it was accessed from carries
/// (`PyEngine::blob()`, `lib.rs`) — an `Arc`-backed clone, never a copy of
/// registry or policy state.
#[pyclass(module = "epistemic_graph.engine", name = "Blob")]
pub(crate) struct PyBlobOps {
    /// Unused until the `blob` lane adds its first `#[pymethods]` that
    /// reads it — kept `pub(crate)` (not private) so `lib.rs`'s
    /// `PyEngine::blob()` accessor can construct this struct directly, the
