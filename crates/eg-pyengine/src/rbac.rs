//! `rbac` domain — pyo3 stub (Wave 0, `EG-PYENGINE-PLAN.md` §4.1). This
//! file exists ONLY so `crates/eg-pyengine/src/lib.rs`'s module tree compiles
//! and `PyEngine::rbac()` has a stable, already-registered return type
//! before the `rbac` Wave-1 lane starts — that lane's own first commit is
...
//! macro DSL, no generic `DomainOps<T>` to produce this file; the `rbac`
...
/// One in-process handle to the `rbac` surface, sharing the SAME
/// registry + authority the `Engine` it was accessed from carries
/// (`PyEngine::rbac()`, `lib.rs`) — an `Arc`-backed clone, never a copy of
/// registry or policy state.
#[pyclass(module = "epistemic_graph.engine", name = "Rbac")]
pub(crate) struct PyRbacOps {
    /// Unused until the `rbac` lane adds its first `#[pymethods]` that
    /// reads it — kept `pub(crate)` (not private) so `lib.rs`'s
    /// `PyEngine::rbac()` accessor can construct this struct directly, the
