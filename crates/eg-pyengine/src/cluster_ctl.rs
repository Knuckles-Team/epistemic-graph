//! `cluster_ctl` domain — pyo3 stub (Wave 0, `EG-PYENGINE-PLAN.md` §4.1). This
//! file exists ONLY so `crates/eg-pyengine/src/lib.rs`'s module tree compiles
//! and `PyEngine::cluster_ctl()` has a stable, already-registered return type
//! before the `cluster_ctl` Wave-1 lane starts — that lane's own first commit is
...
//! macro DSL, no generic `DomainOps<T>` to produce this file; the `cluster_ctl`
...
/// One in-process handle to the `cluster_ctl` surface, sharing the SAME
/// registry + authority the `Engine` it was accessed from carries
/// (`PyEngine::cluster_ctl()`, `lib.rs`) — an `Arc`-backed clone, never a copy of
/// registry or policy state.
#[pyclass(module = "epistemic_graph.engine", name = "ClusterCtl")]
pub(crate) struct PyClusterCtlOps {
    /// Unused until the `cluster_ctl` lane adds its first `#[pymethods]` that
    /// reads it — kept `pub(crate)` (not private) so `lib.rs`'s
    /// `PyEngine::cluster_ctl()` accessor can construct this struct directly, the
