//! `wasm_udf` domain — pyo3 stub (Wave 0, `EG-PYENGINE-PLAN.md` §4.1). This
//! file exists ONLY so `crates/eg-pyengine/src/lib.rs`'s module tree compiles
//! and `PyEngine::wasm_udf()` has a stable, already-registered return type
//! before the `wasm_udf` Wave-1 lane starts — that lane's own first commit is
...
//! macro DSL, no generic `DomainOps<T>` to produce this file; the `wasm_udf`
...
/// One in-process handle to the `wasm_udf` surface, sharing the SAME
/// registry + authority the `Engine` it was accessed from carries
/// (`PyEngine::wasm_udf()`, `lib.rs`) — an `Arc`-backed clone, never a copy of
/// registry or policy state.
#[pyclass(module = "epistemic_graph.engine", name = "WasmUdf")]
pub(crate) struct PyWasmUdfOps {
    /// Unused until the `wasm_udf` lane adds its first `#[pymethods]` that
    /// reads it — kept `pub(crate)` (not private) so `lib.rs`'s
    /// `PyEngine::wasm_udf()` accessor can construct this struct directly, the
