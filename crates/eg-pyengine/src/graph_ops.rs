//! `graph_ops` domain — pyo3 stub (Wave 0, `EG-PYENGINE-PLAN.md` §4.1). This
//! file exists ONLY so `crates/eg-pyengine/src/lib.rs`'s module tree compiles
//! and `PyEngine::graph_ops()` has a stable, already-registered return type
//! before the `graph_ops` Wave-1 lane starts — that lane's own first commit is
...
//! macro DSL, no generic `DomainOps<T>` to produce this file; the `graph_ops`
...
/// One in-process handle to the `graph_ops` surface, sharing the SAME
/// registry + authority the `Engine` it was accessed from carries
/// (`PyEngine::graph_ops()`, `lib.rs`) — an `Arc`-backed clone, never a copy of
/// registry or policy state.
#[pyclass(module = "epistemic_graph.engine", name = "GraphOps")]
pub(crate) struct PyGraphOpsOps {
    /// Unused until the `graph_ops` lane adds its first `#[pymethods]` that
    /// reads it — kept `pub(crate)` (not private) so `lib.rs`'s
    /// `PyEngine::graph_ops()` accessor can construct this struct directly, the
