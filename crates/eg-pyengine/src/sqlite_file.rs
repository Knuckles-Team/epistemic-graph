//! `sqlite_file` domain — pyo3 stub (Wave 0, `EG-PYENGINE-PLAN.md` §4.1). This
//! file exists ONLY so `crates/eg-pyengine/src/lib.rs`'s module tree compiles
//! and `PyEngine::sqlite_file()` has a stable, already-registered return type
//! before the `sqlite_file` Wave-1 lane starts — that lane's own first commit is
...
//! macro DSL, no generic `DomainOps<T>` to produce this file; the `sqlite_file`
...
/// One in-process handle to the `sqlite_file` surface, sharing the SAME
/// registry + authority the `Engine` it was accessed from carries
/// (`PyEngine::sqlite_file()`, `lib.rs`) — an `Arc`-backed clone, never a copy of
/// registry or policy state.
#[pyclass(module = "epistemic_graph.engine", name = "SqliteFile")]
pub(crate) struct PySqliteFileOps {
    /// Unused until the `sqlite_file` lane adds its first `#[pymethods]` that
    /// reads it — kept `pub(crate)` (not private) so `lib.rs`'s
    /// `PyEngine::sqlite_file()` accessor can construct this struct directly, the
