...
fn insert_in(
    ...
    row_insert::insert_rows_in(
        wtx,
        tenant_scope,
        table,
        schema,
        rows,
        |schema, row, rowid| build_insert_cells(schema, col_order, &targets, row, rowid),
        |cells| rmp_serde::to_vec_named(cells).map_err(|e| format!("encode row: {e}")),
    )
}
