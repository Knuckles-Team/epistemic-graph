...
mod tests {
    ...
    #[test]
    fn batched_path_output_is_pinned_for_a_fixed_series() {
        let n = 100;
        let xs: Vec<f64> = (0..n).map(|i| ((i * 37) % n) as f64 * 0.25).collect();
        let ys: Vec<f64> = (0..n).map(|i| (i as f64 * 0.61).cos() * 4.0).collect();
        assert_eq!(
            format!("{:?}", m4_reduce(&xs, &ys, (0.0, 24.75), 7)),
            "[(0.0, 4.0), (2.25, -3.9100430245532984), (3.5, 2.629008475068185), (3.75, 0.6746165658250541), (4.25, 3.969906990313372), (6.5, -3.983904421002773), (7.0, -0.5441572190098277), (7.25, -2.3422516098605124), (8.0, -3.998059689422097), (10.25, 3.93441724095326), (10.5, -3.344305445341367), (10.75, 0.9043336316235512), (11.75, 3.9926952511477833), (12.25, -3.9527292562072023), (14.0, -3.8519464604997453), (14.25, 3.531002000774269), (15.5, -3.9538586474589836), (16.0, 3.9922406400909582), (17.5, 3.933073753770338), (17.75, 3.7371834611355856), (19.25, 3.8818754592330342), (19.75, -3.9982836697242496), (21.0, 3.134183288405189), (21.25, -3.983233298156245), (23.5, 3.9708076842690296), (24.75, -2.8937254396527763)]"
        );
    }

    #[test]
    fn scalar_path_output_is_pinned_including_clamped_and_non_finite_rows() {
        let mut xs: Vec<f64> = (0..30).map(|i| i as f64 - 5.0).collect();
        let mut ys: Vec<f64> = (0..30).map(|i| ((i * 13) % 11) as f64).collect();
        xs[4] = f64::NAN;
        ys[17] = f64::INFINITY;
        assert_eq!(
            format!("{:?}", m4_reduce(&xs, &ys, (0.0, 20.0), 4)),
            "[(-5.0, 0.0), (0.0, 10.0), (4.0, 7.0), (5.0, 9.0), (6.0, 0.0), (9.0, 6.0), (10.0, 8.0), (11.0, 10.0), (13.0, 3.0), (14.0, 5.0), (15.0, 7.0), (17.0, 0.0), (22.0, 10.0), (24.0, 3.0)]"
        );
    }
}
