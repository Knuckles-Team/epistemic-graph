//! # GeoSPARQL baseline (CONCEPT:EG-KG.ontology.concept-10)
//!
...
//! `eg-geo` leaf crate (CONCEPT:EG-KG.ontology.singles-concept/EG-256/EG-258/EG-259) — this module only
//!
...
//! through the SAME dispatch (CONCEPT:EG-KG.ontology.concept-7). Each is a boolean `geof:` FILTER function
//! over two geometry operands, composed from the `eg-geo` DE-9IM predicates
//! (CONCEPT:EG-KG.ontology.de-9im-relations) — including the [`boundaries_intersect`](eg_geo::predicates::boundaries_intersect)
...
/// `geo:asWKT`, `geo:wktLiteral`, …). (CONCEPT:EG-KG.ontology.concept-10)
...
/// which the FILTER dispatch routes here. (CONCEPT:EG-KG.ontology.concept-10)
pub const GEOF_NS: &str = "http://www.opengis.net/def/function/geosparql/";

/// The `geo:wktLiteral` datatype IRI (CONCEPT:EG-KG.ontology.concept-10).
...
/// this base (e.g. `…/metre`, `…/degree`). (CONCEPT:EG-KG.ontology.concept-10)
pub const UOM_NS: &str = "http://www.opengis.net/def/uom/OGC/1.0/";

/// Parse a `geo:wktLiteral` lexical form into an [`eg_geo::Geometry`] (CONCEPT:EG-KG.ontology.concept-10).
///
...
/// (CONCEPT:EG-KG.ontology.concept-10). Returns `None` for an unknown function or unparseable geometry —
...
/// predicates (CONCEPT:EG-KG.ontology.de-9im-relations); `sfEquals` uses the spatial-equality predicate, not
...
/// `b`'s boundary — a *tangential* proper part (CONCEPT:EG-KG.ontology.concept-7).
...
/// (lon/lat degrees → metres, CONCEPT:EG-KG.ontology.concept-8); otherwise the planar Euclidean
/// coordinate distance (CONCEPT:EG-KG.ontology.singles-concept) is returned (the `…/degree` UOM). Returns `None`
...
/// `geof:buffer(geom, radius, units)` (CONCEPT:EG-KG.ontology.concept-10): the geometry grown by `radius`,
/// returned as a WKT lexical form suitable for a `geo:wktLiteral`. Lowers onto
/// `eg-geo`'s planar `buffer` (CONCEPT:EG-KG.ontology.concept-9) — the `units` argument is accepted but
...
/// spelling mentioning "met" (metre/meter). (CONCEPT:EG-KG.ontology.concept-10)
...
mod tests {
    ...
    // ── RCC8 / Egenhofer fixtures (CONCEPT:EG-KG.ontology.concept-7) ─────────────────────────────────
