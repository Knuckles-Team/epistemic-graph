...
/// The per-vertex result of a distributed graph computation, sorted by vertex id.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[cfg_attr(feature = "contract-schema", derive(schemars::JsonSchema))]
pub enum DistResult {
    /// PageRank: `(vertex_id, score)` rows.
    Scores(Vec<(String, f64)>),
    /// Connected components / BFS: `(vertex_id, label)` rows; a component label is
    /// the index of its representative in the sorted vertex list.
...
impl DistResult {
    ...
    pub fn len(&self) -> usize {
        match self {
            DistResult::Scores(rows) => rows.len(),
            DistResult::Labels(rows) => rows.len(),
        }
    }
