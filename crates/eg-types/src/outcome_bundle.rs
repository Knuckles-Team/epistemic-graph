...
fn validate_receipt_node(
    ...
    validate_receipt_properties(bundle, node)?;
    Ok(())
}
...
fn validate_receipt_properties(
    bundle: &CommitOutcomeBundle,
    node: &ReceiptNode,
) -> Result<(), String> {
    ...
    if kind != receipt_kind_name(node.kind) {
    ...
    if completeness != completeness_name(bundle.completeness) {
        return Err(format!(
...
impl RunEvent {
    ...
    pub fn validate_for_bundle(&self, bundle: &CommitOutcomeBundle) -> Result<(), String> {
        self.validate()?;
        if !event_identity_matches_bundle(self, bundle) {
...
fn event_identity_matches_bundle(event: &RunEvent, bundle: &CommitOutcomeBundle) -> bool {
