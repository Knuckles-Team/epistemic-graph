...
//! a [`ModelSpec`] becomes a usable solver model only through the validating
//! constructor in `eg_compute::solve`, which checks every field and lowers
//! every typed constraint to one linear row.
